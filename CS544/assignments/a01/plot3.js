...
// The code for the third visualization is contained in this file.
// It is a scatter plot of the score.js data. 
// The ACT score is on the y-axis, while the x-axis is the total of the SAT scores. 
// The circle's color corresponds to its GPA. D3.js is utilized to build the visualization.
...
let circles = div3.selectAll("circle")
    .data(scores)
    .enter()
    .append("circle")
    .attr("cx", score => ((score.SATM + score.SATV) / 3.6) + 20)
    .attr("cy", score => (450 - (score.ACT) * 11))
    .attr("r", score => score.GPA)
    .attr("fill", score => {
        // Get color on the basis of GPA
        if (score.GPA >= 3.5)
            return '#628395';
        else if (score.GPA < 3.5 && score.GPA >= 3)
            return '#96897b';
        else if (score.GPA < 3 && score.GPA >= 2.5)
            return '#d0ce7c';
        else
            return '#dbad6a';
    })
