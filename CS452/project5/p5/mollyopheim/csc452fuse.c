...
/*
* Called when the root directory is null, and we need to read from the 
* disk file to get the root directory
*/
static void getroot() {
	printf("root testing\n");
	FILE *fileptr;
	...
	if(fileptr != NULL) {
		fread(&root, sizeof(struct csc452_root_directory), 1, fileptr);
		printf("%d is the number of directories\n", root.nDirectories);
	}
	fclose(fileptr);
...
static int csc452_getattr(const char *path, struct stat *stbuf)
{
	...
	// initializing char arrays to correct size bc I don't want to 
	// overrun strings/buffers
...
*/	sscanf(path, "/%[^/]/%[^.].%s", directory, filename, extension);
	...
	printf("%s is the path in getattr\n", path);
		...
		int exists = -1;
		getroot();
		//if(in directories array from rootdirectory struct
		for(int i = 0; i < MAX_DIRS_IN_ROOT; i++) {
			if(strcmp(root.directories[i].dname, directory) == 0) {
				printf("the directory already existed\n");
				stbuf->st_mode = S_IFDIR | 0755;
				stbuf->st_nlink = 2;
				// because the dir exists we return 0
				return res;
			}
			 ...
			 off_t offset, struct fuse_file_info *fi)
{
	...
	// use scanf to see if we're looking for a file in a dir
	// or just a dir
	...
	// if it's a file we want to return
	...
	if (strcmp(path, "/") == 0) {
		...
		// look the dirs in the root if we just have a dir
		for(int i = 0; i < MAX_DIRS_IN_ROOT; i++) {
			if(strcmp(root.directories[i].dname, directory) == 0) {
				// use filler to list the contents
...
static int csc452_mkdir(const char *path, mode_t mode)
{
	...
	// -EPERM
	printf("%s is the path in mkdir\n", path);

	printf("%s is the filename in mkdir\n", filename);


	// EEXIST
	for(int i = 0; i < MAX_DIRS_IN_ROOT; i++) {
		if(strcmp(root.directories[i].dname, directory) == 0) {
			printf("found the dir that already exists\n");
			return -EEXIST;
		}
	}
	...
	// where we want to write to in the .disk file
	long startBlock = (long) root.nDirectories;
	csc452_directory_entry dir; 
	dir.nFiles = 0;
	root.nDirectories += 1;
	strcpy(root.directories[root.nDirectories -2].dname,directory);
	root.directories[root.nDirectories-2].nStartBlock = startBlock;
	// now that we have the directory entry we want to 
	// put it on the disk

	FILE *file = fopen(".disk", "r+b");
	// we have to move the file pointer past the root
