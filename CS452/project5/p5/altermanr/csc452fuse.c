...
int cur_file_contents_indx = -1;

int get_file_index(const char* path) {
	path++;

	for(int i = 0; i < cur_file_indx; i++) {
...
void add_dir(const char* name) {
	cur_dir_indx++;
	strcpy(dir_list[cur_dir_indx], name);
}

void add_file(const char* name) {
	cur_file_indx++;
	strcpy(files_list[cur_file_indx], name);

	cur_file_contents_indx++;
	strcpy(file_contents[cur_file_contents_indx], "");
}
...
void write_to_file(const char* path, const char* content) {
	...
	strcpy(file_contents[index], content);
}
			  ...
			  struct fuse_file_info *fi)
{
	...
	char* content = file_contents[index];
