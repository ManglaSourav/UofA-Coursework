...
// Purpose: The following program is file system using fuse.
...
{
	int res = 0;
	char dir[MAX_FILENAME+1], name[MAX_FILENAME+1], exten[MAX_EXTENSION+1];
...
static int csc452_mkdir(const char *path, mode_t mode)
{
	...
	sscanf(path,"/%[^/]/%[^.].%s",dir,name,exten);

	if(strlen(dir) > MAX_FILENAME) { //error check name
		return -ENAMETOOLONG;
	}

	//create root
	FILE *disk = read_disk();
	fseek(disk, 0 * BLOCK_SIZE, SEEK_SET);
	void *block = malloc(BLOCK_SIZE);
	fread(block, BLOCK_SIZE, 1, disk);
  	csc452_root_directory *rootDir = block;
	return 0;
}
