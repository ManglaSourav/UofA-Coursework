...
{
	csc452_node* newNode = (csc452_node*)kmalloc(sizeof(csc452_node), GFP_KERNEL);
	if(newNode == NULL)
	{
		return -1;
	}
...
SYSCALL_DEFINE3(csc452_get_msg, const char __user*, to, char __user*, msg, const char __user*, from)
{
	csc452_node* cur = head;
	...
	// Remove the message
	csc452_node* prev;
	csc452_node* temp = head;
	cur = head;

	if(temp != NULL && temp->to == to)
	{
		head = temp->next;
		kfree(temp);
	}
