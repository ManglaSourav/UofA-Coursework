...
{
	struct queue_node  *newNode = kmalloc(sizeof(struct queue_node), GFP_KERNEL);
	if (newNode == NULL){
		return -1;
	}
	newNode->username = kstrdup(to, GFP_KERNEL);
	newNode->sender = kstrdup(from, GFP_KERNEL);
	newNode->message = kstrdup(msg, GFP_KERNEL);
	newNode->next = NULL;
...
SYSCALL_DEFINE3(csc452_get_msg, char*, to, char*, msg, char*, from)
{
	...
	struct queue_node *ptr2;
	struct queue_node *prev = NULL;
	int flag = 0;
	for (ptr = head; ptr != NULL; ptr = ptr->next){
		if (strcmp(to, ptr->username) == 0 && flag == 0){
		   strcpy(msg, ptr->message);
		   strcpy(from, ptr->sender);
		   kfree(ptr->sender);
		   kfree(ptr->username);
		   kfree(ptr->message);
		   flag++;
		...
		}else if (strcmp(to,ptr->username)==0 && flag != 0){
		   return 1;
		}
		prev = ptr;
	}
