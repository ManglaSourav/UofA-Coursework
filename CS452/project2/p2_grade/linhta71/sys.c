...
/**
* Author: Linh Ta
* define the syscall for sending the message, which add message to queue
*/
SYSCALL_DEFINE3(csc452_send_msg, const char __user *, to, const char __user *, msg, const char __user *, from) {
	...
	newNode->from = kmalloc(strlen(from)+1, GFP_KERNEL);
	newNode->to = kmalloc(strlen(to)+1, GFP_KERNEL);
	newNode->msg = kmalloc(strlen(msg)+1, GFP_KERNEL);
	newNode->next = NULL;
	if (newNode->from == NULL || newNode->to== NULL || newNode->msg == NULL) {
		return -1;
	}
...
/**
* Author: Linh Ta
* this define a syscall to get all the messages sent to a user, the return value
* will be copy to the parameter from and msg
*/
