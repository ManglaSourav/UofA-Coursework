...
{
	struct msg_node *cur;
	struct msg_node *temp;

	cur = kmalloc(sizeof(struct msg_node), GFP_KERNEL); 
	strcpy(cur->to, to);
...
SYSCALL_DEFINE3(csc452_get_msg, const char __user *, to, char __user *, msg, char __user *, from)
{
	...
	while (temp != NULL) {
		if (strncmp(temp->to,to,strlen(to)) == 0) {
			if (temp->next != NULL) {
	...
	temp2 = head;
	while(temp2 != NULL) {
		if (strncmp(temp2->to,to,strlen(to)) == 0) {
			kfree(temp);
			return 1;
		}
		temp2 = temp2->next;
...
/*
 * Ugh. T avoid negative return values, "getpriority()" will
