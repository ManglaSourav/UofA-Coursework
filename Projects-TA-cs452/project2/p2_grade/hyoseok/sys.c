...
{
	struct message send_this = {.from=from, .msg=msg};
	struct message *temp;

	if (head==NULL) {
		// empty linked list
		head = (struct message *)kmalloc(sizeof(struct message),GFP_KERNEL);
		head = send_this;
	} else {
		// linked list already has nodes inside
...
{
	// give all messages received
	struct message *temp;
	temp = (struct message *)kmalloc(sizeof(struct message), GFP_KERNEL);
	temp = head;
	if (temp->next == NULL) {
		// only head in this linked list
		printf("%s said: %s\n", temp->from, temp->msg);
	} else {
		while (temp->next != NULL) {
			printf("%s said: %s\n", temp->from, temp->msg);
